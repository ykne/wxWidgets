...
public:
    ...
    //
    // Note: m_appIndicatorMenu must be declared before m_appIndicator as the
    // latter needs to be destroyed first to ensure it doesn't keep a dangling
    // GtkMenu pointer to the former.
...
{
#if wxUSE_APPINDICATOR
    wxBitmap bmp = m_win ? m_bitmap.GetBitmapFor(m_win) : m_bitmap.GetBitmap(wxDefaultSize);
    if (!bmp.IsOk())
        return;
